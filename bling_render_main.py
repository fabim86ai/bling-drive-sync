...
def get_drive_service():
      try:
                ...
                if not creds_json: raise ValueError("GOOGLE_CREDENTIALS_JSON nao configurada")
                          try: creds_info = json.loads(base64.b64decode(creds_json).decode('utf-8'))
      except Exception: creds_info = json.loads(creds_json)
...
def sync_job():
    ...
    if not drive_service: return {"error": "Falha Drive"}
...
def sync_now(): return jsonify(sync_job())
